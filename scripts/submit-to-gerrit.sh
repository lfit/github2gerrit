...
query_gerrit_changes() {
    ...
    while IFS= read -r cid; do
        ...
        # Query Gerrit via SSH
        if ssh -v -n -p 29418 "$GERRIT_SSH_USER_G2G@$GERRIT_SERVER" \
            "gerrit query limit:1 owner:self is:open project:$PROJECT_REPO_GERRIT --current-patch-set --format=JSON ${cid##* }" \
